...
public class LinkedList : IEnumerable<int>
{
    ...
    public void Remove(int value)
    {
        ...
        while (curr is not null)
        {
            if (curr.Data == value)
            {
                // If we're removing the only node in the list (head == tail),
                // or if the node is at the head, use RemoveHead
                if (curr == _head)
                {
                    RemoveHead();
                }
                // If the node is at the tail (but not the only node), use RemoveTail
                else if (curr == _tail)
                {
                    RemoveTail();
                }
    ...
    public void Replace(int oldValue, int newValue)
    {
        ...
        while (curr is not null)
        {
            // If current node's data matches oldValue, replace it with newValue
